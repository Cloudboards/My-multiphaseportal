 // --- STEP BY STEP PROCESSING ARCHITECTURE ---
    ...
    nextToPhase2.addEventListener('click', () => {
        ...
        if (fNameClean === "" || lNameClean === "" || mNameClean === "" || eNameClean === "" || aNameClean === "" || gNameClean === "" || dNameClean === "" ) {
            ...
            validationAlert.classList.remove('d-none');
            validationAlert.textContent = "Validation Failure: Please Enter A Valid Name"
            return;
        }
        else if (aNameClean > 65) {
            validationAlert.classList.remove('d-none');
            validationAlert.textContent = "Validation Failure: Age must be less or equal than 65"
            return;
        }
        else if (aNameClean < 18) {
            validationAlert.classList.remove('d-none');
            validationAlert.textContent = "Validation Failure: Age must be above or equal than 18"
            return;
        }
    ...
    nextToPhase3.addEventListener('click', () => {
        ...
        const formattedDateOfBirth = dateOfBirth.value.trim().toUpperCase();

        // Target injections handling
        previewName.textContent = `${formattedLast}, ${formattedFirst} ${formattedMiddle}${formattedExt}`;
        previewDemo.textContent = `${formattedAge}, ${formattedGender}, ${formattedDateOfBirth}`;
    ...
    restartBtn.addEventListener('click', () => {
        ...
        currentAddress.value = ""; permanentAddress.value = ""; age.value = ""; gender.value = "";
        dateOfBirth.value = "";
